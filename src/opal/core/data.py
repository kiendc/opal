...
class Data:
    ...
    Data is one of two elementary entities. It has a name, value type and
    value. The most important methods of this class are `set` and `get`.
...
class DataTable:
    ...
    def get_row(self, rowId):
        ...
        if rowId not in self.table.keys():
            return valueDict  # Return an empty dictionary
    ...
    def get_column(self, colId):
        ...
        for row in self.row_identities:
            if row in self.table.keys() and colId in self.table[row].keys():
