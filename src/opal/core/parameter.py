...
class Parameter(Data):
    ...
    def __init__(self, kind='real', default=None, bound=None, name=None,
        ...
        if kind == 'real' and isinstance(default, float):
            raise ValueError, 'default value must agree with type'

        if kind == 'integer' and isinstance(default, int):
            raise ValueError, 'default value must agree with type'
...
class ParameterConstraint:
    """
    Represent for a simple constraint of parameters. Only parameters involve
    to this type of constraint.
    This is prior constraint, this means that the constraints will be
    validated before runing the algorithm.
    """
    violated = False
