...
class NOMADBlackbox(Environment):
    ...
    NOMADBlackbox object represent a NOMAD blackbox that is kind of an
    interface between the NOMAD solver and model (problem). Because of the
    fact that the used NOMAD solver require that an blackbox is an executable
    ...
    we design in such a way that the executable will initialize an
    NOMADBlackbox object too. The advantage that we minimize the generated
...
class NOMADSolver(Solver):
    ...
    def __init__(self, name='NOMAD',
                 parameterFile='nomad-param.txt',
                 **kwargs):
    ...
    def generate_neighbors_executable(self,
        ...
        bb.write('import logging' + endl)
        bb.write('from opal.Solvers.nomad import NOMADBlackbox' + endl)
        bb.write('from opal.core.structureevaluator ' +
                 'import FunctionEvaluator' +
                 endl)
        bb.write('worker = FunctionEvaluator(' +
                 'name="neighborhood-function evaluator", ' +
                 'functionFile="' +
                 dataFile + '")' + endl)
        bb.write(comment + 'Create model evaluation environment' + endl)
    ...
    def create_specification_file(self, 
        ...
        if model.bounds is not None:
            lowerBoundStr = str([bound[0] 
                                 for bound in model.bounds 
                                 if bound is not None]
                                ).replace('None', '-').replace(',', ' ')
            upperBoundStr = str([bound[1] 
                                 for bound in model.bounds 
                                 if bound is not None]
                                ).replace('None', '-').replace(',', ' ')
